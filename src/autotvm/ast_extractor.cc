...
class IndexvarCollector: public StmtExprVisitor {
 public:
  void Collect(PrimExpr expr) {
    this->VisitExpr(expr);
  }

  void VisitStmt_(const VarNode *op) {
    vars.insert(op);
  }
...
class ASTExtractor : public FeatureVisitor {
 public:
  void Extract(Stmt stmt, std::shared_ptr<Tree> root,
               const std::unordered_map<Var, ItervarFeature,
 ...
 private:
  ...
  const std::unordered_map<Var, ItervarFeature,
                           tvm::ObjectPtrHash, tvm::ObjectPtrEqual> *itervar_map_;
...
TVM_REGISTER_GLOBAL("autotvm.feature.GetSimplifiedAST")
